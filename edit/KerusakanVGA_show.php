			...
			<center>
				...
				<?php
					...
					if (isset($_GET['aksi'])) {
						if ($_GET['aksi'] == 'hapus') {
							...
							$db->deleteKerusakanVGA($id);
						}elseif ($_GET['aksi'] == 'tambah') {
						echo"<br>
							<form method=POST action='?aksi=tambahKerusakanVGA'>
								<table>
							...
							";
						} elseif ($_GET['aksi'] == 'tambahKerusakanVGA') {
							$id=$_POST['id'];
							$solper = $_POST['solper'];
							...
							$db->tambahKerusakanVGA($id, $solper, $benar, $salah, $mulai, $selesai);
				...
				?>	
							<center><form method="post" action="<?php $_SERVER['PHP_SELF'] ?>?aksi=update">
								<table>
									<tr><td>ID</td><td>:</td><td><input type='text' name='id' value="<?php echo $db->bacaKerusakanVGA('ID', $id); ?>" size="13" disabled></td><input type="hidden" name="id" value="<?php echo $id; ?>"></tr>
									
									<tr><td>Solusi dan Pertanyaan</td><td>:</td><td><textarea name='solper' cols='40' required><?php echo $db->bacaKerusakanVGA('solusi_dan_pertanyaan', $id); ?></textarea></td></tr>
									
									<tr><td>Bila benar</td><td>:</td><td><input type="text" name="bila_benar" value="<?php echo $db->bacaKerusakanVGA('bila_benar', $id); ?>" size="13" required></td></tr>
									
									<tr><td>Bila salah</td><td>:</td><td><input type="text" name="bila_salah" value="<?php echo $db->bacaKerusakanVGA('bila_salah', $id); ?>" size="13" required></td></tr>	
									
									<tr><td>Mulai</td><td>:</td><td><input type="text" name="mulai" value="<?php echo $db->bacaKerusakanVGA('mulai', $id); ?>" size="1" required></td></tr>	
									
									<tr><td>Selesai</td><td>:</td><td><input type="text" name="selesai" value="<?php echo $db->bacaKerusakanVGA('selesai', $id); ?>" size="1" required></td></tr> 
						...
						<?php
								...
								$db->updateKerusakanVGA($id, $solper, $benar, $salah, $mulai, $selesai);
							}
					}
						?>			
			</center>
			<?php
				$arrayKerusakanVGA = $db->tampilKerusakanVGA();
				...
				<center>
					...
					foreach ($arrayKerusakanVGA as $data) {
