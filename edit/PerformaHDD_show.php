			...
			<center>
				...
				<?php
					...
					if (isset($_GET['aksi'])) {
						if ($_GET['aksi'] == 'hapus') {
							...
							$db->deletePerformaHDD($id);
						}elseif ($_GET['aksi'] == 'tambah') {
						echo"<br>
							<form method=POST action='?aksi=tambahPerformaHDD'>
								<table>
							...
							";
						} elseif ($_GET['aksi'] == 'tambahPerformaHDD') {
							$id=$_POST['id'];
							$solper = $_POST['solper'];
							...
							$db->tambahPerformaHDD($id, $solper, $benar, $salah, $mulai, $selesai);
				...
				?>	
							<center><form method="post" action="<?php $_SERVER['PHP_SELF'] ?>?aksi=update">
								<table>
									<tr><td>ID</td><td>:</td><td><input type='text' name='id' value="<?php echo $db->bacaPerformaHDD('ID', $id); ?>" size="13" disabled></td><input type="hidden" name="id" value="<?php echo $id; ?>"></tr>
									
									<tr><td>Solusi dan Pertanyaan</td><td>:</td><td><textarea name='solper' cols='40' required><?php echo $db->bacaPerformaHDD('solusi_dan_pertanyaan', $id); ?></textarea></td></tr>
									
									<tr><td>Bila benar</td><td>:</td><td><input type="text" name="bila_benar" value="<?php echo $db->bacaPerformaHDD('bila_benar', $id); ?>" size="13" required></td></tr>
									
									<tr><td>Bila salah</td><td>:</td><td><input type="text" name="bila_salah" value="<?php echo $db->bacaPerformaHDD('bila_salah', $id); ?>" size="13" required></td></tr>	
									
									<tr><td>Mulai</td><td>:</td><td><input type="text" name="mulai" value="<?php echo $db->bacaPerformaHDD('mulai', $id); ?>" size="1" required></td></tr>	
									
									<tr><td>Selesai</td><td>:</td><td><input type="text" name="selesai" value="<?php echo $db->bacaPerformaHDD('selesai', $id); ?>" size="1" required></td></tr> 
						...
						<?php
								...
								$db->updatePerformaHDD($id, $solper, $benar, $salah, $mulai, $selesai);
							}
					}
						?>			
			</center>
			<?php
				$arrayPerformaHDD = $db->tampilPerformaHDD();
				...
				<center>
					...
					foreach ($arrayPerformaHDD as $data) {
