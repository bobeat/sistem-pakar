...
<body>
		<div id="content">
		<h2><a href="index_konsul.php">Konsultasi</a> > Kerusakan ATA/SATA Hard Drive</h2><hr>
			<?php
				include("database.php");
			?>
			<?php 
			if(!isset($_GET['idpertanyaan'])){
				$sqlp = "select * from kerusakan_hdd where mulai='Y'";
				$detail=mysql_query("select kerusakan_hdd.solusi_dan_pertanyaan, detail_kerusakanhdd.nama_detail from kerusakan_hdd, detail_kerusakanhdd where kerusakan_hdd.ID=detail_kerusakanhdd.ID_detail");
				$rs=mysql_query($sqlp);
				...
				$sqlp = "select * from kerusakan_hdd where ID=$idsolusi";
				$detail=mysql_query("select kerusakan_hdd.solusi_dan_pertanyaan, detail_kerusakanhdd.nama_detail from kerusakan_hdd, detail_kerusakanhdd where detail_kerusakanhdd.ID=$idsolusi");
				$rs=mysql_query($sqlp);
				$data=mysql_fetch_array($rs);
				$dadet=mysql_fetch_array($detail);				
				echo "<form>
