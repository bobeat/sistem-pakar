...
<body>
		<div id="content">
		<h2><a href="index_konsul.php">Konsultasi</a> > Kerusakan VGA</h2><hr>
			<?php
				include("database.php");
			?>
			<?php 
			if(!isset($_GET['idpertanyaan'])){
				$sqlp = "select * from kerusakan_vga where mulai='Y'";
				$detail=mysql_query("select kerusakan_vga.solusi_dan_pertanyaan, detail_kerusakanvga.nama_detail from kerusakan_vga, detail_kerusakanvga where kerusakan_vga.ID=detail_kerusakanvga.ID_detail");
				$rs=mysql_query($sqlp);
				...
				$sqlp = "select * from kerusakan_vga where ID=$idsolusi";
				$detail=mysql_query("select kerusakan_vga.solusi_dan_pertanyaan, detail_kerusakanvga.nama_detail from kerusakan_vga, detail_kerusakanvga where detail_kerusakanvga.ID=$idsolusi");
				$rs=mysql_query($sqlp);
				$data=mysql_fetch_array($rs);
				$dadet=mysql_fetch_array($detail);				
				echo "<form>
