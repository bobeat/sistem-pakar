...
<body>
		<div id="content">
		<h2><a href="index_konsul.php">Konsultasi</a> > Performa ATA/SATA Hard Drive</h2><hr>
			<?php
				include("database.php");
			?>
			<?php 
			if(!isset($_GET['idpertanyaan'])){
				$sqlp = "select * from performa_hdd where mulai='Y'";
				$detail=mysql_query("select performa_hdd.solusi_dan_pertanyaan, detail_performahdd.nama_detail from performa_hdd, detail_performahdd where performa_hdd.ID=detail_performahdd.ID_detail");
				$rs=mysql_query($sqlp);
				...
				$sqlp = "select * from performa_hdd where ID=$idsolusi";
				$detail=mysql_query("select performa_hdd.solusi_dan_pertanyaan, detail_performahdd.nama_detail from performa_hdd, detail_performahdd where detail_performahdd.ID=$idsolusi");
				$rs=mysql_query($sqlp);
				$data=mysql_fetch_array($rs);
				$dadet=mysql_fetch_array($detail);				
				echo "<form>
