...
<body>
		<div id="content">
		<h2><a href="index_konsul.php">Konsultasi</a> > Kerusakan Motherboard, RAM dan Prosesor</h2><hr>
			<?php
				include("database.php");
			?>
			<?php 
			if(!isset($_GET['idpertanyaan'])){
				$sqlp = "select * from kerusakan_mobo where mulai='Y'";
				$detail=mysql_query("select kerusakan_mobo.solusi_dan_pertanyaan, detail_kerusakanmobo.nama_detail from kerusakan_mobo, detail_kerusakanmobo where kerusakan_mobo.ID=detail_kerusakanmobo.ID_detail");
				$rs=mysql_query($sqlp);
				...
				$sqlp = "select * from kerusakan_mobo where ID=$idsolusi";
				$detail=mysql_query("select kerusakan_mobo.solusi_dan_pertanyaan, detail_kerusakanmobo.nama_detail from kerusakan_mobo, detail_kerusakanmobo where detail_kerusakanmobo.ID=$idsolusi");
				$rs=mysql_query($sqlp);
				$data=mysql_fetch_array($rs);
				$dadet=mysql_fetch_array($detail);				
				echo "<form>
