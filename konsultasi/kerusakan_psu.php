...
<body>
		<div id="content">
		<h2><a href="index_konsul.php">Konsultasi</a> > Kerusakan Power Supply</h2><hr>
			<?php
				include("database.php");
			?>
			<?php 
			if(!isset($_GET['idpertanyaan'])){
				$sqlp = "select * from kerusakan_psu where mulai='Y'";
				$detail=mysql_query("select kerusakan_psu.solusi_dan_pertanyaan, detail_kerusakanpsu.nama_detail from kerusakan_psu, detail_kerusakanpsu where kerusakan_psu.ID=detail_kerusakanpsu.ID_detail");
				$rs=mysql_query($sqlp);
				...
				$sqlp = "select * from kerusakan_psu where ID=$idsolusi";
				$detail=mysql_query("select kerusakan_psu.solusi_dan_pertanyaan, detail_kerusakanpsu.nama_detail from kerusakan_psu, detail_kerusakanpsu where detail_kerusakanpsu.ID=$idsolusi");
				$rs=mysql_query($sqlp);
				$data=mysql_fetch_array($rs);
				$dadet=mysql_fetch_array($detail);				
				echo "<form>
