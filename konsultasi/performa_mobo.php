...
<body>
		<div id="content">
		<h2><a href="index_konsul.php">Konsultasi</a> > Performa Motherboard, RAM, dan Prosesor</h2><hr>
			<?php
				include("database.php");
			?>
			<?php 
			if(!isset($_GET['idpertanyaan'])){
				$sqlp = "select * from performa_mobo where mulai='Y'";
				$detail=mysql_query("select performa_mobo.solusi_dan_pertanyaan, detail_performamobo.nama_detail from performa_mobo, detail_performamobo where performa_mobo.ID=detail_performamobo.ID_detail");
				$rs=mysql_query($sqlp);
				...
				$sqlp = "select * from performa_mobo where ID=$idsolusi";
				$detail=mysql_query("select performa_mobo.solusi_dan_pertanyaan, detail_performamobo.nama_detail from performa_mobo, detail_performamobo where detail_performamobo.ID=$idsolusi");
				$rs=mysql_query($sqlp);
				$data=mysql_fetch_array($rs);
				$dadet=mysql_fetch_array($detail);				
				echo "<form>
